...
bool IskhakovDGrahamConvexHullMPI::ValidationImpl() {
  ...
  return input_size >= 3 && input_size <= static_cast<size_t>(std::numeric_limits<int>::max());
}
...
std::vector<Point> IskhakovDGrahamConvexHullMPI::GrahamScan(const std::vector<Point> &input_points) {
  ...
  std::sort(points.begin() + 1, points.end(), [&pivot, &orientation](const Point &a, const Point &b) {
    ...
    if (o == 0) {
      const double dist1 = (a.x - pivot.x) * (a.x - pivot.x) + (a.y - pivot.y) * (a.y - pivot.y);
      const double dist2 = (b.x - pivot.x) * (b.x - pivot.x) + (b.y - pivot.y) * (b.y - pivot.y);
      return dist1 < dist2;
    }
    return o == 2;
  });
...
int IskhakovDGrahamConvexHullMPI::CalculateOptimalActiveProcs(int points_count, int world_size) {
  ...
  while (optimal_active_procs > 1 && points_count / optimal_active_procs < kMinPointsPerProcess) {
...
std::vector<Point> IskhakovDGrahamConvexHullMPI::PrepareAndDistributeData(int world_rank, int world_size,
  ...
  const int optimal_active_procs = CalculateOptimalActiveProcs(points_count, world_size);
  optimal_active_procs_out = optimal_active_procs;

  const bool is_active = (world_rank < optimal_active_procs);
  const int base_points_count = points_count / optimal_active_procs;
  const int remainder = points_count % optimal_active_procs;
  ...
  if (world_rank == 0) {
    ...
    for (int i = 0; i < world_size; ++i) {
      if (i < optimal_active_procs) {
        const int proc_count = base_points_count + (i < remainder ? 1 : 0);
  ...
  if (world_rank == 0) {
    all_x.resize(static_cast<size_t>(points_count));
    all_y.resize(static_cast<size_t>(points_count));
    for (int i = 0; i < points_count; ++i) {
      all_x[static_cast<size_t>(i)] = GetInput()[static_cast<size_t>(i)].x;
      all_y[static_cast<size_t>(i)] = GetInput()[static_cast<size_t>(i)].y;
    }
  }

  std::vector<double> local_x(static_cast<size_t>(my_points_count));
  std::vector<double> local_y(static_cast<size_t>(my_points_count));
  ...
  local_points.reserve(static_cast<size_t>(my_points_count));
  for (int i = 0; i < my_points_count; ++i) {
    local_points.emplace_back(local_x[static_cast<size_t>(i)], local_y[static_cast<size_t>(i)]);
  }
...
std::vector<Point> IskhakovDGrahamConvexHullMPI::MergeHullsBinaryTree(int world_rank,
  ...
  for (int step = 1; step < optimal_active_procs; step *= 2) {
    ...
    if (world_rank < optimal_active_procs && partner < optimal_active_procs) {
      const int my_hull_size = static_cast<int>(current_hull.size());
      ...
      if (my_hull_size > 0 && partner_hull_size == 0) {
        std::vector<double> hull_x(static_cast<size_t>(my_hull_size));
        std::vector<double> hull_y(static_cast<size_t>(my_hull_size));
        for (int i = 0; i < my_hull_size; ++i) {
          hull_x[static_cast<size_t>(i)] = current_hull[static_cast<size_t>(i)].x;
          hull_y[static_cast<size_t>(i)] = current_hull[static_cast<size_t>(i)].y;
        }
        ...
        std::vector<double> remote_x(static_cast<size_t>(partner_hull_size));
        std::vector<double> remote_y(static_cast<size_t>(partner_hull_size));
        ...
        remote_hull.reserve(static_cast<size_t>(partner_hull_size));
        for (int i = 0; i < partner_hull_size; ++i) {
          remote_hull.emplace_back(remote_x[static_cast<size_t>(i)], remote_y[static_cast<size_t>(i)]);
        }
        current_hull = std::move(remote_hull);
      } else if (my_hull_size > 0 && partner_hull_size > 0) {
        std::vector<double> hull_x(static_cast<size_t>(my_hull_size));
        std::vector<double> hull_y(static_cast<size_t>(my_hull_size));
        for (int i = 0; i < my_hull_size; ++i) {
...
std::vector<Point> IskhakovDGrahamConvexHullMPI::BroadcastFinalResult(int world_rank,
  ...
  std::vector<double> final_x(static_cast<size_t>(final_hull_size));
  std::vector<double> final_y(static_cast<size_t>(final_hull_size));

  if (world_rank == 0) {
    final_result = final_hull_root;
    for (int i = 0; i < final_hull_size; ++i) {
      final_x[static_cast<size_t>(i)] = final_result[static_cast<size_t>(i)].x;
      final_y[static_cast<size_t>(i)] = final_result[static_cast<size_t>(i)].y;
    }
  }
  ...
  if (world_rank != 0) {
    final_result.reserve(static_cast<size_t>(final_hull_size));
    for (int i = 0; i < final_hull_size; ++i) {
      final_result.emplace_back(final_x[static_cast<size_t>(i)], final_y[static_cast<size_t>(i)]);
    }
  }
