...
std::vector<Point> IskhakovDGrahamConvexHullMPI::GrahamScan(const std::vector<Point> &input_points) {
  ...
  for (size_t i = 1; i < points.size(); i++) {
    if (points[i].y < points[index_min_point].y ||
        (std::abs(points[i].y - points[index_min_point].y) < kEpsilon &&
...
int IskhakovDGrahamConvexHullMPI::CalculateOptimalActiveProcs(int points_count, int world_size) {
  ...
  while (optimal_active_procs > 1 && 
         points_count / optimal_active_procs < MIN_POINTS_PER_PROCESS) {
    optimal_active_procs--;
  }

  while (optimal_active_procs > 1 && 
         points_count / optimal_active_procs < 3) {
    optimal_active_procs--;
  }
...
std::vector<Point> IskhakovDGrahamConvexHullMPI::PrepareAndDistributeData(int world_rank, int world_size,
  ...
  if (my_points_count > 0) {
    MPI_Scatterv(all_x.data(), sendcounts.data(), displacements.data(), MPI_DOUBLE,
                 local_x.data(), my_points_count, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatterv(all_y.data(), sendcounts.data(), displacements.data(), MPI_DOUBLE,
                 local_y.data(), my_points_count, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  }
  ...
  for (int step = 1; step < optimal_active_procs; step *= 2) {
    ...
    if (world_rank < optimal_active_procs && partner < optimal_active_procs) {
      ...
      if (my_hull_size > 0 && partner_hull_size == 0) {
        ...
        MPI_Recv(remote_x.data(), partner_hull_size, MPI_DOUBLE, partner, 1,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Recv(remote_y.data(), partner_hull_size, MPI_DOUBLE, partner, 2,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);
